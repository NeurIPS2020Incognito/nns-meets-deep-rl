...
class ParallelHNSW(HNSW):
    def __init__(self, graph, k=1, ef=1, max_trajectory=80, batch_size=500000,
        ...
        # Service labels to denote service fields and boundary situations TODO: better description
    ...
    def prepare_edges_with_probs(self, agent, state=None, is_evaluate=False, greedy=False, **kwargs):
        ...
        self.num_pos_confident = 0
        self.num_neg_confident = 0
        ...
        for i in range(len(self.from_vertex_ids)):
            ...
            self.num_pos_confident += (self.edge_confidence[i] == self.edge_patience).sum()
            self.num_neg_confident += (self.edge_confidence[i] == -self.edge_patience).sum()
        ...
        if not greedy:
            print('Number of pos confident edges: ', self.num_pos_confident)
            print('Number of neg confident edges: ', self.num_neg_confident)
        return edges, probs
